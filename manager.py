...
parser = argparse.ArgumentParser(
    description="Manage administrative tasks", formatter_class=CustomHelpFormatter
)
...
admin_parser.add_argument(
    "--username", required=True, help="Username for the administrator account"
)
admin_parser.add_argument(
    "--password", required=True, help="Password for the administrator account"
)
admin_parser.add_argument(
    "--is_active",
    type=str2bool,
    ...
    const=True,
    default=True,
    help="Activate the administrator account",
)
admin_parser.add_argument(
    "--email", help="Email address for the administrator (Optional)"
)

# --- Project Management ---
project_parser = subparsers.add_parser(
    "create-project", help="Create a new project", formatter_class=CustomHelpFormatter
)

project_parser.add_argument("--title", required=True, help="Project Title")
project_parser.add_argument(
    "--start_date", required=True, help="Project Start Date (dd/mm/yyyy)"
)
...
task_parser = subparsers.add_parser(
    "add-task", help="Add a new task", formatter_class=CustomHelpFormatter
)
...
task_parser.add_argument(
    "--duration", default=1, type=int, help="Duration in days (Optional)"
)
task_parser.add_argument(
    "--priority",
    choices=["CRITICAL", "HIGH", "MEDIUM", "LOW"],
    default="LOW",
    help="Task priority (Optional)",
)
task_parser.add_argument(
    "--status",
...
task_parser = subparsers.add_parser(
    "move-task",
    help="Move a task to a different status",
    formatter_class=CustomHelpFormatter,
)
task_parser.add_argument("--project_id", required=True, help="Project ID")
...
task_parser = subparsers.add_parser(
    "delete-task", help="Delete a task", formatter_class=CustomHelpFormatter
)
task_parser.add_argument("--project_id", required=True, help="Project ID")
task_parser.add_argument("--task_id", required=True, help="Task ID")

task_parser = subparsers.add_parser(
    "assign-member", help="Assign a user to a task", formatter_class=CustomHelpFormatter
)
task_parser.add_argument("--project_id", required=True, help="Project ID")
task_parser.add_argument("--task_id", required=True, help="Task ID")
task_parser.add_argument(
    "--username", required=True, help="Username of the member to assign"
)

# --- Member Management ---
member_parser = subparsers.add_parser(
    "add-member", help="Add a user to a project", formatter_class=CustomHelpFormatter
)
...
task_parser = subparsers.add_parser(
    ...
    help="Remove a user from a task",
    formatter_class=CustomHelpFormatter,
)
task_parser.add_argument("--project_id", required=True, help="Project ID")
...
class UserManager(DataManager):
    ...
    def create_user(
        ...
        if email and is_active is not None:
            print(
                f"[blue italic]User account created: username='{username}', password='{password}', Email='{email}', Is_active='{is_active}'[/]"
            )
        elif email:
            print(
                f"[blue italic]User account created: username='{username}', password='{password}', Email='{email}'[/]"
            )
        else:
            print(
                f"[blue italic]User account created: username='{username}', password='{password}', No email provided[/]"
            )
        return user
...
class TaskManager(DataManager):
    ...
    def add_task(
        ...
        hashed_task_id = bcrypt.hashpw(
            task_id.encode("utf-8"), bcrypt.gensalt()
        ).decode("utf-8")
        task = {
...
if __name__ == "__main__":
    ...
    if args.command == "create-user":
        user_manager.create_user(
            args.username, args.password, args.is_active, args.email, True
        )
    elif args.command == "purge-data":
        data_manager.purge_data(
            input("[bold red]Are you sure you want to erase all data? (y/n): ")
        )
        ...
        task_manager.remove_assignee_from_task(
            args.project_id, args.task_id, args.username
        )
